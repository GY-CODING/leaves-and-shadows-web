...
import { ASGARD_IMG, FLEX_COLUMN, FLEX_ROW_CENTER, GROVE_OF_FETTERS_IMG } from '@/utils/global.constants'
...
export default function Home (): JSX.Element {
  ...
  return (
    <div className='w-screen h-[300vh] overflow-x-hidden flex flex-col m-0 p-0 jusity-start items center z-0'>
      ...
      <Box sx={{ width: '100%', height: '100vh', zIndex: -1, position: 'absolute', filter: 'grayscale(90%)', opacity: 0.1 }}>
        <Image
      ...
      />
          ...
          <Box sx={[FLEX_ROW_CENTER, {
            ...
            <Box sx={{ width: '90%', aspectRatio: 16 / 9 }} component={'img'} src={ASGARD_IMG}/>
          </Box>
          <Box sx={[FLEX_COLUMN, {
            width: '35%',
            height: '100%',
            ...
            <Box sx={[FLEX_ROW_CENTER, { width: '100%', height: '20%', gap: '2rem', zIndex: 3 }]}>
            <Typography onClick={handleChangeToGame} variant='h5' sx={{ cursor: 'pointer', padding: 2, fontFamily: 'cinzel', fontWeight: 'bold', color: isGameSelected ? 'white' : 'gray', borderBottom: isGameSelected ? '2px solid white' : '2px solid transparent', transition: '.5s' }}>
                Juego
              </Typography>
              <Typography onClick={handleChangeToHistory} variant='h5' sx={{ cursor: 'pointer', padding: 2, fontFamily: 'cinzel', fontWeight: 'bold', color: isGameSelected ? 'gray' : 'white', borderBottom: !isGameSelected ? '2px solid white' : '2px solid transparent', transition: '.5s' }}>
                Historia
              </Typography>
            </Box>
            <Box sx={[FLEX_COLUMN, { width: '100%', height: '50%' }]}>
            <Typography sx={{ fontFamily: 'cinzel', color: 'white', transition: '.5s', textAlign: 'justify', textJustify: 'distribute' }}>
